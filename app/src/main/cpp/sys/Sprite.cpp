...
uint32_t	Sprite::spr_color[4];			// カラー
...
/**************************************
    設定
		引数	_tex    = テクスチャ
				...
				_origin = 原点位置
 **************************************/
void	Sprite::set(Texture* _tex, Rect<short> const* _coord, int _origin)
{
	_tex->load();
	texture	= _tex;								// テクスチャ
...
/***********************************
    カラー設定
		引数	_color = カラー値
 ***********************************/
void	Sprite::set_color(uint32_t _color)
{
	spr_color[0] = spr_color[1] = spr_color[2] = spr_color[3] = _color;
}

void	Sprite::set_color(uint32_t const* _color)
{
...
{
	if ( shader ) {
		Renderer::use_shader(shader);						// シェーダ
	}
	if ( texture ) {				// テクスチャ有り
		texture->bind();									// テクスチャ
