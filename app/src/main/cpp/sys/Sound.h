#ifndef	___SYS_SOUND_H___
#define	___SYS_SOUND_H___
...
static const size_t		FILE_ASSET = 0xffffffff;			// assetファイル指定
	...
	int				loop;			// ループカウンタ
	long			position;		// 再生位置
	std::unique_ptr<uint8_t>	file_data;					// 終了時に解放するデータ
	SoundData*		next;			// 次のデータ

		SoundData(const void*, size_t, int);	// コンストラクタ
		~SoundData()							// デストラクタ
		{
			if ( next ) {
				delete	next;
...
enum
{
	...
	void	play(const void*, uint32_t, int _loop = 1, float _vol = 1.0f);		// 再生
	void	play(void);
	void	prepare(const void*, uint32_t, int _loop = 1, float _vol = 1.0f);	// 再生準備
	void	prepare(void);
	void	set_next(const void*, uint32_t, int _loop = 1);						// 連続再生設定
...
struct SoundCommand
{
	...
	void	set(int _channel, int _command, const void* _data, uint32_t _size, int _loop, float _volume)
			{
				channel	= _channel;
				command	= _command;
				data	= _data;
...
static const int	QUEUE_SIZE = 32;
	...
	static void		set_command(int, int, const void* _dat = nullptr, uint32_t _size = 0, int _loop = 0, float _volume = 0.0f);
												// コマンド予約
...
public :
	...
	static void		resume_manager(void);		// 初期化
	static void		pause_manager(void);		// 停止
	static void		release(void);				// 終了
	static void		update(void);				// 稼働

	static void		play(int, const void*, uint32_t, int _loop = 1, float _vol = 1.0f);			// 再生
	...
	static void		prepare(int, const void*, uint32_t, int _loop = 1, float _vol = 1.0f);		// 再生準備
	...
	static void		set_next(int, const void*, uint32_t, int _loop = 1);						// 連続再生設定
