...
{
	static const
	char*	tex_name[TEX_MAX - TEX_PARTS] =
	{
		"sprite/parts.png",
		"sprite/logo.png",
		"sprite/button.png",
	...
	texture = new sys::CTexture[TEX_MAX];				// テクスチャ
	for (int i = TEX_PARTS; i < TEX_MAX; i++) {
		texture[i].set(tex_name[i - TEX_PARTS]);
	}
	...
	SprDef	spr_def[SPR_MAX - SPR_SHADOW] =
			{
				{TEX_PARTS,		{ 24,  24, 100, 100}},			// パネル陰影
				{TEX_PARTS,		{ 20,   0, 108,   8}},			// 軌跡用ライン（横）
				{TEX_PARTS,		{  0,  24,   8, 108}},			// 軌跡用ライン（縦）
				{TEX_PARTS,		{  0,   0,   8,   8}},			// 軌跡用ポイント
				{TEX_PARTS,		{ 20,  12, 108,   8}},			// 解答用ライン（横）
				{TEX_PARTS,		{ 12,  24,   8, 108}},			// 解答用ライン（縦）
				{TEX_PARTS,		{  0, 136,  64,  64}},			// カーソル
				{TEX_LOGO,		{  0,   0, 218,  82}},			// タイトルロゴ
				{TEX_LOGO,		{220,   0, 218,  82}},
				{TEX_LOGO,		{  0,  88, 440, 120}},			// "CLEAR"
				{-1,			{  0,   0, 480, 852}},			// ポーズメニュー下地
				{TEX_BUTTON,	{  0,   0, 400,  64}},			// "かんたん"
				{TEX_BUTTON,	{  0,  64, 400,  64}},			// "ふつう"
				{TEX_BUTTON,	{  0, 128, 400,  64}},			// "むずかしい"
				{TEX_BUTTON,	{  0, 652, 400,  64}},			// "フリーモード"
				{TEX_BUTTON,	{196, 720, 192,  96}},			// "おすすめアプリ"
				{TEX_BUTTON,	{  0, 720, 192,  96}},			// "Playゲーム"
				{TEX_BUTTON,	{  0, 256, 400,  64}},			// "キャンセル"
				{TEX_BUTTON,	{  0, 320, 400,  64}},			// "やりなおし"
				{TEX_BUTTON,	{  0, 384, 400,  64}},			// "解答表示"
				{TEX_BUTTON,	{  0, 448, 400,  64}},			// "解答非表示"
				{TEX_BUTTON,	{  0, 512, 400,  64}},			// "ゲーム終了"
				{TEX_BUTTON,	{  0, 584, 200,  64}},			// "メニュー"
			};
	...
	for (i = SPR_SHADOW; i < SPR_MAX; i++) {
		if ( spr_def[i - SPR_SHADOW].tex < 0 ) {
			sprite[i].set(spr_def[i - SPR_SHADOW].coord.w, spr_def[i - SPR_SHADOW].coord.h);
		}
		else if ( spr_def[i - SPR_SHADOW].coord.w > 0 ) {
			sprite[i].set(&texture[spr_def[i - SPR_SHADOW].tex], &spr_def[i - SPR_SHADOW].coord);
		}
		else {
			sprite[i].set(&texture[spr_def[i - SPR_SHADOW].tex]);
		}
	}
	...
	sys::Sound::play(0, "sound/bgm_menu.ogg", 0);		// BGM再生開始
	fade_in();
	clear_cnt = -1;
	menu_cnt  = 8;
	phase     = PHASE_START;
...
/************
    メイン
 ************/
sys::Scene*		SceneGame::update(void)
{
	if ( recommend_state ) {			// おすすめアプリ表示中
		if ( (key_status & KEY_BACK) || (key_dialog & KEY_CANCEL) ) {
			play_se(SE_BACK);
			call_advertisement(ADVERTISEMENT_WALL + 1);
			sys::Sound::set_volume(0, 1.0f);
			recommend_state = false;
		}
		return	this;
	}
	else if ( dialog_state ) {			// ダイアログ表示中
		if ( key_dialog & KEY_YES ) {					// アプリ終了
			sys::Sound::stop();
			return	nullptr;
		}
		else if ( key_dialog & KEY_CANCEL ) {			// 戻る
			sys::Sound::set_volume(0, 1.0f);
			dialog_state = false;
		}
		else {
	...
	else if ( key_status & KEY_BACK ) {					// バックキー
		play_se(SE_BACK);
		sys::Sound::set_volume(0, 0.7f);
		open_dialog();
		dialog_state = true;
		return	this;
	}
	...
	switch ( phase ) {
	  ...
	  case PHASE_START + 1 :
		if ( --undo_cnt == 0 ) {
			...
			init_button();										// ボタン初期化
			sys::Sound::play(0, "sound/bgm_game.ogg", 0);		// BGM再生開始
			fade_in(300);
			phase = PHASE_GAME;
		}
		break;
...
void	SceneGame::start_game(int _level)
{
	...
	static char		_file[32];

	sprintf(_file, "sprite/back%02d.pkm", back_num);	// 背景画像
	texture[TEX_BACK].set(_file);
	for (int i = 0; i < 4; i++) {						// 背景画像
		sprite[SPR_FRAME + i].set(&texture[TEX_BACK], &spr_rect[i]);
		sprite[SPR_FRAME + i].set_origin(sys::Sprite::X_LEFT | sys::Sprite::Y_TOP);
	}
	for (i = 0; i < FIELD_H; i++) {						// パネル画像設定
		for (j = 0; j < FIELD_W; j++) {
			panel[i][j].set(&texture[TEX_BACK]);
		}
	}
...
void	SceneGame::draw(void)
{
	...
	if ( (phase > PHASE_START + 1) && (clear_cnt < 30) ) {	// カーソル
		...
		for (i = 0; i < 8; i++) {
			...
			sprite[SPR_CURSOR].draw(_x, _y, _scl, _scl, ((common_counter + i*4) % 96)*(M_PI*2/96));
		}
		sys::Sprite::set_color();
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	}
	...
	if ( clear_cnt > 30 ) {									// "CLEAR"
		...
		sprite[SPR_CLEAR].draw(0.0f, -244.0f + sinf((common_counter % 240)*(M_PI*2/240))*8.0f);
		sys::Sprite::set_color();
	}
}
